...
飞书图片推送：`scripts/send_focus_card_feishu.py` 默认优先发送关注卡 PNG。配置 `FEISHU_APP_ID` + `FEISHU_APP_SECRET`（开通 `im:resource`）时可上传 `image_key` 并在群内直接显示图片；仅有 Webhook 时先尝试短时图片链接按钮卡片，公共图床不可用则回退飞书 Markdown / 文本卡片，避免日更静默失败。群内直接出图仍需配置飞书应用凭证。
